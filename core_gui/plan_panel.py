...
class PlanPanelMixin:
    ...
    def _on_run_plan(self):
        ...
        if self.assistant_callback:
            # Показуємо кнопку Стоп, ховаємо Виконати
            self.plan_run_btn.pack_forget()
            self.plan_stop_btn.pack(side='right', padx=(0, 5))
            self.assistant_callback('run_plan', None)

    def _on_stop_plan(self):
        """Обробник кнопки 'Стоп план'."""
        if self.assistant_callback:
            self.assistant_callback('stop_plan', None)
        # Повертаємо кнопку Виконати
        self.plan_stop_btn.pack_forget()
        self.plan_run_btn.pack(side='right', padx=(0, 5))

    def on_plan_execution_started(self):
    ...
    def on_plan_execution_finished(self):

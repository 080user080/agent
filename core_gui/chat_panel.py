...
class ChatPanelMixin:
    ...
    def add_message(self, sender, message):
        ...
        if current_text and sender == "assistant" and message.strip().startswith('{'):
            # Перевірити чи останній непустий рядок теж починається з {
            ...
            if last_nonempty and last_nonempty.startswith('{'):
                skip_separator = True
        # Пропускаємо роздільник для коротких системних повідомлень (STT/TTS статус)
        if current_text and sender == "assistant":
            # Якщо поточне або останнє повідомлення - короткий системний статус
        ...
        # Відправник (пропускаємо префікс при об'єднанні JSON-блоків)
        ...
        if sender == "assistant":
            ...
            if hasattr(self, 'status_var'):
                ...
                # Зберігаємо інфо про LLM якщо є, додаємо час
                if "(" in current_status and "с)" in current_status:
                    self.status_var.set(f"{current_status} | {ts}")
                else:
    ...
    def on_input_blur(self, event=None):
        ...
        current_text = self.input_text.get(1.0, tk.END).strip()
        # Не показуємо placeholder взагалі - поле залишається порожнім
        # if not current_text:
    ...
    def on_input_key(self, event=None):
        ...
        self.last_input_time = time.time()
        # Динамічне збільшення висоти поля вводу
        # Використовуємо after для того щоб текст вже був вставлений
        if self.root:
            self.root.after(50, self._auto_resize_input_text)  # Збільшено затримку до 50мс

    def _auto_resize_input_text(self):
        """Автоматично збільшує висоту поля вводу залежно від кількості рядків."""
        ...
        try:
            ...
            if not text or text.strip() == '':
                ...
                # Спробуємо отримати реальну кількість рядків через dlineinfo
                try:
                    last_index = self.input_text.index(tk.END)
                    # Отримуємо номер останнього рядка
                    last_line = int(last_index.split('.')[0])
                    lines = last_line
                except:
                    # Fallback: підраховуємо через newline characters
                    lines = text.count('\n') + 1
                    # Якщо текст довгий без переносів, враховуємо ширину
                    if '\n' not in text:
                        # Приблизний розрахунок: кожні ~50 символів = новий рядок
                        lines = max(1, len(text) // 50 + 1)
                # Обмежуємо між 1 і 6 рядками
                new_height = max(1, min(6, lines))
            
            current_height = int(self.input_text['height'])
            if new_height != current_height:
                self.input_text.config(height=new_height)
        except Exception as e:
            pass  # Помилка при зміні розміру - ігноруємо
    ...
    def send_text_command(self):
        ...
        # Guard від дублювання: якщо та ж команда була відправлена < 5с тому — ігноруємо
        now = time.time()
        last_cmd = getattr(self, '_last_sent_command', None)
        last_time = getattr(self, '_last_sent_time', 0)
        if last_cmd == command and (now - last_time) < 5.0:
            self.input_text.delete(1.0, tk.END)
            self._auto_resize_input_text()  # Скинути висоту після очищення
            return
    ...
    def end_stream_message(self):
        ...
        self.chat_history.see(tk.END)
        self.chat_history.configure(state='disabled')
        # Скинути флаг стрімінгу
        self._is_streaming = False
        if hasattr(self, 'status_var'):
            current_status = self.status_var.get()
            ts = _time.strftime('%H:%M:%S')
            # Зберігаємо інфо про LLM якщо є, додаємо "Відповідь готова"
            if "(" in current_status and "с)" in current_status:
                self.status_var.set(f"{current_status} | Відповідь готова | {ts}")
            else:
                self.status_var.set(f"✅ Відповідь готова | {ts}")

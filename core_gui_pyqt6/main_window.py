    ...
    def _on_message(self, msg_type: str, data: Any) -> None:
        ...
        try:
            if msg_type == "add_message":
                ...
                if self.settings_tab and hasattr(self.settings_tab, 'update_watcher_status_from_executor'):
                    mw = self.assistant
                    if mw and hasattr(mw, 'windsurf_watcher'):
                        self.settings_tab.update_watcher_status_from_executor(mw.windsurf_watcher)
                self.update_progress(0, "🌀 Windsurf Watch запущено")
                ...
                reason = state.get("stop_reason", data.get("reason", "")) if isinstance(data, dict) else ""
                self.update_progress(0, f"⏹️ Windsurf Watch зупинено ({reason})")
            elif msg_type == "windsurf_response":
                if self.settings_tab and hasattr(self.settings_tab, '_windsurf_watch_status'):
                    pass  # Оновлюється через update_watcher_status_from_executor
                self.update_progress(0, "🌀 Отримано нову відповідь Windsurf")
            elif msg_type == "windsurf_error":
                error = data.get("error", "") if isinstance(data, dict) else str(data)
                self.update_progress(0, f"❌ Windsurf Watch помилка: {error}")

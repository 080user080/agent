...
class SettingsTab(BaseTab):
    ...
    def _build_settings_tab(self) -> None:
        ...
        # Додаємо stretch спочатку, щоб контент був зверху
        ...
        # --- Нижня панель: статус + кнопки (поза QScrollArea) ---
        bottom_bar = QWidget()
        bottom_bar.setObjectName("bottom_bar")
        bottom_bar_layout = QVBoxLayout(bottom_bar)
        bottom_bar_layout.setContentsMargins(4, 2, 4, 2)
        bottom_bar_layout.setSpacing(2)

        # Статус (перманентний, видимий завжди)
        ...
        bottom_bar_layout.addWidget(self._settings_status)

        # --- Компактна панель кнопок ---
        buttons_bar = QFrame()
        ...
        # ─── Кнопка Windsurf Watch ────────────────────────────────────────────
        self._windsurf_watch_btn = QPushButton("🌀 Windsurf Watch ▶")
        self._windsurf_watch_btn.setStyleSheet(_COMPACT_BTN_STYLE + "background: #0f6f73; color: white;")
        self._windsurf_watch_btn.setCheckable(True)
        self._windsurf_watch_btn.clicked.connect(self._toggle_windsurf_watch)
        btn_bar_layout.addWidget(self._windsurf_watch_btn)
    ...
    def _on_category_changed(self, row: int) -> None:
        ...
        # Видалити старий контент (крім stretch)
    ...
    def _clear_right_content(self) -> None:
        """Видалити всі віджети категорій з правої панелі."""
    ...
    # ─── Windsurf Watch Toggle ─────────────────────────────────────────────────

    def _toggle_windsurf_watch(self) -> None:
        """Перемкнути Windsurf Watch Start/Stop."""
        mw = self._main_window
        assistant = mw.assistant if mw else None

        if not assistant:
            self._update_settings_status("❌ Асистент не доступний", "#d32f2f")
            self._windsurf_watch_btn.setChecked(False)
            return

        if self._windsurf_watch_btn.isChecked():
            # Start
            if not hasattr(assistant, 'start_windsurf_watch'):
                self._update_settings_status("⚠️ Windsurf Watch не підтримується", "#e65100")
                self._windsurf_watch_btn.setChecked(False)
                return
            try:
                assistant.start_windsurf_watch()
                self._windsurf_watch_btn.setText("🌀 Windsurf Watch ■")
                self._windsurf_watch_btn.setStyleSheet(
                    "font: 9pt 'Segoe UI'; font-weight: bold; padding: 2px 8px; "
                    "border: none; border-radius: 3px; min-height: 24px; max-height: 26px;"
                    "background: #c62828; color: white;"
                )
                self._update_settings_status("▶️ Windsurf Watch запущено", "#2e7d32")
                # Оновлюємо статус watcher-ів
                if hasattr(assistant, 'windsurf_watcher'):
                    self.update_watcher_status_from_executor(assistant.windsurf_watcher)
            except Exception as e:
                self._update_settings_status(f"❌ Помилка запуску: {e}", "#d32f2f")
                self._windsurf_watch_btn.setChecked(False)
        else:
            # Stop
            if hasattr(assistant, 'stop_windsurf_watch'):
                try:
                    assistant.stop_windsurf_watch()
                except Exception as e:
                    self._update_settings_status(f"❌ Помилка зупинки: {e}", "#d32f2f")
            self._windsurf_watch_btn.setText("🌀 Windsurf Watch ▶")
            self._windsurf_watch_btn.setStyleSheet(
                "font: 9pt 'Segoe UI'; font-weight: bold; padding: 2px 8px; "
                "border: none; border-radius: 3px; min-height: 24px; max-height: 26px;"
                "background: #0f6f73; color: white;"
            )
            self._update_settings_status("⏹️ Windsurf Watch зупинено", "#1976d2")
            # Оновлюємо статус watcher-ів
            if hasattr(assistant, 'windsurf_watcher'):
                self.update_watcher_status_from_executor(assistant.windsurf_watcher)

    def update_watcher_status_from_executor(self, executor: Any = None) -> None:
        """Оновити статус Watcher-ів з WindsurfWatcherExecutor."""
        if not self._watcher_status_text:
            return
        text = self._watcher_status_text
        text.clear()
        if executor is None:
            text.append("Windsurf Watcher: не доступний")
            return
        try:
            state = executor.get_state() if hasattr(executor, 'get_state') else {}
            is_running = state.get("is_running", False)
            responses = state.get("responses_captured", 0)
            snapshots = state.get("snapshots_taken", 0)
            if is_running:
                text.append(f"🟢 Windsurf Watcher: активний")
            else:
                text.append(f"🔴 Windsurf Watcher: зупинено")
            text.append(f"   Відповідей: {responses}, Снепшотів: {snapshots}")
            if state.get("stop_reason"):
                text.append(f"   Причина: {state['stop_reason']}")
        except Exception as e:
            text.append(f"⚠️ Помилка отримання статусу: {e}")

    # ─── Watcher статус (загальний) ───────────────────────────────────────────

...
class TestPresets:
    def test_lmstudio_defaults(self):
        p = lmstudio_provider()
        assert p.base_url == "http://localhost:1234/api/v1/chat"
        assert p.api_key is None
        assert p.capabilities.offline is True
        assert p.name == "lmstudio"

    def test_lmstudio_openai_compatible_mode(self):
        p = lmstudio_provider(use_native_api=False)
        assert p.base_url == "http://localhost:1234/v1/chat/completions"
        assert p.api_key is None
        assert p.capabilities.offline is True
        assert p.name == "lmstudio"

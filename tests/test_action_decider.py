...
class TestActionDeciderDecide:
    ...
    def test_decide_handles_llm_error(self):
        ...
        d = _make_decider(llm_fn=llm_fn,)
        action = d.decide(goal="test", observation=None, history=[])
        # LLM error → fallback на take_screenshot (агресивне відновлення)
        assert action.name == "take_screenshot"
        assert "fallback" in action.reasoning
    ...
    def test_decide_handles_llm_exception(self):
        ...
        d = _make_decider(llm_fn=llm_fn)
        action = d.decide(goal="test", observation=None, history=[])
        # LLM exception → fallback на take_screenshot (агресивне відновлення)
        assert action.name == "take_screenshot"
        assert "fallback" in action.reasoning
...
class TestAgentLoopWithDecider:
    ...
    def test_decider_done_terminates_loop(self):
        ...
        # summary знаходиться в args["summary"] (через _plan_from_decider)
        assert plan["args"]["summary"] == "all good"
        assert plan["args"]["success"] is True
        assert plan["success"] is True

    def test_decider_fallback_to_take_screenshot(self):
        # Decider returns error → fallback на take_screenshot (агресивне відновлення)
        ...
        plan = loop.plan("test", Observation(), AgentState())
        # Якщо decider повертає take_screenshot — це тепер очікувана поведінка
        # (раніше був noop, але агресивне відновлення take_screenshot краще)
        assert plan["action"] in ("take_screenshot", "noop")
        assert plan["from_decider"] is True
    ...
    def test_consecutive_failures_increment(self):
        ...
        class FailingRegistry:
            ...
            def execute_function(self, name, args, auto_create=False):
                self.call_count += 1
        ...
        # Виконання закінчиться через max_steps або через done
        assert result["ok"] is False  # Всі дії провалилися
        assert result["steps"] >= 1  # Хоча б один крок зроблено
...
class TestAgentLoopLongTasks:
    ...
    def test_general_task_with_ask_user(self):
        ...
        assert result["ok"] is True
        # ask_user викликається через registry, потім read_code_file
        assert len(registry.calls) == 2
        assert registry.calls[0][0] == "ask_user"
        assert registry.calls[1][0] == "read_code_file"

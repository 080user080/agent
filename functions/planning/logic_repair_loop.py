...
class StepRepairer:
    ...
    def is_available(self) -> bool:
        ...
        if self.assistant is None:
            return False
        # Перевірка що assistant має робочий ask_llm
        if not hasattr(self.assistant, 'ask_llm') and not hasattr(self.assistant, 'get_llm'):
            return False
        # Якщо ask_llm є, але це заглушка — теж недоступно
        try:
            ask_llm = getattr(self.assistant, 'ask_llm', None)
            if ask_llm is None:
                ask_llm = getattr(self.assistant, 'get_llm', None)
            if ask_llm is None:
                return False
            # Якщо ask_llm є викликається як функція — вважаємо що робочий
        except Exception:
            return False
        return True

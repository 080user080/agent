...
if TYPE_CHECKING:
    from functions.planning.logic_task_learner import TaskPattern
...
class SelfLearning:
    ...
    def learn_from_pattern(self, pattern: "TaskPattern") -> str:
        """Інтеграція з logic_task_learner — зберегти виявлений патерн як skill.

        Args:
            pattern: TaskPattern з logic_task_learner

        Returns:
            skill_id або порожній рядок
        """
        if not pattern or not pattern.steps:
            return ""
        # Конвертуємо патерн у skill
        skill_id = f"pattern_{len(self.skills) + 1}"
        self.skills[skill_id] = {
            "id": skill_id,
            "name": pattern.name,
            "pattern": " — ".join(s.get("action", "?") for s in pattern.steps[:3]),
            "action": json.dumps(pattern.steps, ensure_ascii=False),
            "metadata": {
                "frequency": pattern.frequency,
                "steps_count": len(pattern.steps),
                "last_seen": datetime.fromtimestamp(pattern.last_seen).isoformat(),
                "source": "logic_task_learner",
            },
            "created_at": datetime.now().isoformat(),
            "usage_count": 0,
        }
        self._save_json(self.skills_file, self.skills)
        print(f"✅ Pattern learned as skill: {pattern.name} ({len(pattern.steps)} steps)")
        return skill_id

...
if PYQT6_AVAILABLE:
    ...
    class VoiceTrayIcon(QObject):  # type: ignore[no-redef]
        ...
        def _get_tooltip(self, status: VoiceStatus, text: str = "") -> str:
            ...
            try:
                ...
                hotkey_display = "Ctrl+F9"

            status_texts = {
                VoiceStatus.IDLE: f"🎤 Готовий (натисніть {hotkey_display})",
    ...
    class VoiceTrayIcon:  # type: ignore[no-redef]
        """Заглушка для оточень без PyQt6."""
        _update_requested = None
        status_changed = None
        current_status = VoiceStatus.IDLE
        ...
        def initialize(self) -> bool:
            print("[TrayIcon] PyQt6 не доступний - заглушка")
            return False

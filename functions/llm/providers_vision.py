...
class VisionLMProvider:
    ...
    def describe(self, image_path: str, prompt: str = "") -> str:
        """Отримати текстовий опис зображення через Vision-LM.

        Args:
            image_path: шлях до зображення
            prompt: додатковий промпт для опису

        Returns:
            Текстовий опис або порожній рядок при помилці.
        """
        if not self._available:
            return ""

        question = prompt or "Опиши що видно на цьому зображенні одним абзацом."
        query = VisionQuery(image_path=image_path, question=question)
        try:
            response = self.analyze_image(query)
            return response.text or ""
        except Exception as e:
            logger.warning("Vision-LM describe error: %s", e)
            return ""

    def detect_ui_elements(self, image_path: str) -> List[Dict[str, Any]]:
        """Детектувати UI елементи на скріншоті.

        RETURN:  # TODO: Partial implementation — MVP stub
            [{"type": "button", "name": "Submit", "rect": {...}}, ...]
        NOTE:  This method returns an empty list because the structured 
               parsing of vision responses is not yet implemented.
               See `_parse_vision_elements()` — planned for Phase 5.
        """
        if not self._available:
            return []
    ...
    def suggest_actions(self, image_path: str, goal: str) -> List[str]:
        ...
        NOTE:  This method returns response.suggested_actions from the LLM
               response, but structured parsing is not implemented yet.
               The `_parse_vision_actions()` method is planned for Phase 5.
        """
        if not self._available:
            return []

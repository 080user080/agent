...
def list_directory(directory=None):
    ...
    try:
        ...
        # Формуємо повідомлення
        lines_out = [f"--- START OF DIRECTORY LIST ({len(items)} items) ---"]
        lines_out.append(f"📁 ПОВНИЙ ВМІСТ ПАПКИ {directory}:")
        for item in items:
            if item["is_dir"]:
        ...
        lines_out.append("--- END OF DIRECTORY LIST (ALL FILES SHOWN) ---")

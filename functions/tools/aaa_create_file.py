...
def create_file(filename=None, content="", filepath=None, path=None, name=None, save_path=None):
    """Створити файл (txt/py).

    Підтримує кілька ім'я аргументу (`filename`, `filepath`, `path`, `name`, `save_path`),
    бо LLM іноді плутається у назві параметра.
    """
    try:
        # Нормалізуємо аргумент імені файлу
        target = filename or filepath or path or name or save_path
        if not target:

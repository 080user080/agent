...
from .logic_repair_loop import RepairLoop, RepairProposer
...
class TaskRunner:
    ...
    def __init__(
        ...
        *,
        ...
        repair_proposer: Optional[RepairProposer] = None,
        ask_user_callback: Optional[Callable[[str, List[str]], str]] = None,
        cwd: Optional[str] = None,
        auto_approve_read_only: bool = False,
    ):
        self.gate = gate or PermissionGate(auto_approve_read_only=auto_approve_read_only)
        ...
        self.repair_loop = RepairLoop(repair_proposer) if repair_proposer else None
        ...
        for attempt in range(attempts):
            ...
            if task.expect and status == STATUS_OK:
                ...
                if not all_ok(expect_results):
                    ...
                    # ----- Phase 12.2: Repair loop на expect_failed -----
                    if self.repair_loop:
                        repair_action, modified_args = self.repair_loop.repair_failed_step(
                            expect_results, task, report
                        )
                        metadata["repair_action"] = repair_action.value

                        if repair_action == "retry" and modified_args:
                            # Retry з новими аргументами
                            logger.info("Repair: retry task з новими аргументами")
                            task.params.update(modified_args)
                            # Перевиконуємо handler
                            try:
                                result = handler(ctx)
                                if not isinstance(result, dict):
                                    result = {"summary": str(result)}
                                # Перевіряємо expect ще раз
                                expect_results = self._evaluate_expectations(
                                    task.expect, task=task, report=report,
                                    previous=previous, handler_result=result,
                                )
                                if all_ok(expect_results):
                                    status = STATUS_OK
                                    result["summary"] = result.get("summary", "") + " (repair successful)"
                                    del result["error"]
                                else:
                                    result["summary"] = result.get("summary", "") + " (repair retry failed)"
                            except Exception as exc:
                                result = {
                                    "status": STATUS_ERROR,
                                    "error": f"repair retry failed: {type(exc).__name__}: {exc}",
                                }

                        elif repair_action == "skip":
                            # Пропускаємо крок
                            logger.info("Repair: skip task")
                            status = STATUS_SKIPPED
                            result["summary"] = result.get("summary", "") + " (repair: skipped)"
                            del result["error"]

                        elif repair_action in ("stop", "replan"):
                            # Зупиняємо виконання
                            logger.info("Repair: stop/replan — зупиняємо виконання")
                            result["summary"] = result.get("summary", "") + f" (repair: {repair_action.value})"
                            # Не змінюємо статус — залишаємо STATUS_EXPECT_FAILED
...
def _handler_ask_user(ctx: TaskContext) -> Dict[str, Any]:
    """Запитує користувача про вхід."""
    import logging
    logger = logging.getLogger("logic_task_runner")

    params = ctx.task.params or {}
    question = params.get("question", "Питання?")
    options = params.get("options", [])  # Список опцій для вибору

    if ctx.runner.ask_user_callback:
        try:
            answer = ctx.runner.ask_user_callback(question, options)
